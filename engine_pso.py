...
def executar_pso(funcao, inercia_tipo, coop_tipo, n_particulas, dim, max_iter, c1, c2):
    ...
    # lista para guardar o melhor fitness de cada iteração
    historico_fitness = []
    ...
    for t in range(max_iter):
        ...
        for i in range(n_particulas):
            ...
            for d in range(dim):
                ...
                p.velocidade[d] = (w * p.velocidade[d] + c1 * r1 * (p.pBest[d] - p.posicao[d]) + c2 * r2 * (alvo[d] - p.posicao[d]))
                p.posicao[d] = p.posicao[d] + p.velocidade[d]
        
        # guarda o melhor resultado encontrado até esta iteração
        historico_fitness.append(melhor_fitness_global)

    # agora retorna o valor final E a lista com o progresso
    return melhor_fitness_global, historico_fitness